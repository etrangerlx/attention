    ...
    def construct(self):
        ...
        for g in range(4):
            ...
            for i in range(3):
                n = i+int(i >= g)
                c = chunks_cp[i]
                anims.append(c.animate.move_to(symmem[g][g].get_left() + (i+1)*0.33*RIGHT))
        self.play(*anims)
        ...
        for i in reversed(range(1,3)):
            ...
            for g in range(4):
                chunks_cp = all_chunks_cp[g]
                n = i-1
                new_text = str(int(chunks_cp[i].submobjects[1].text) + int(chunks_cp[n].submobjects[1].text))
                anims.append(
                        AnimationGroup(
                            Transform(chunks_cp[n].submobjects[1],
                                      Text(new_text).scale(0.35).move_to(chunks_cp[n].submobjects[1])
                                      ),
                            Transform(chunks_cp[i],
                                      Group(Text(new_text).scale(0.35).move_to(chunks_cp[n].submobjects[1])),
                                      remover=True
                                      ),
                            )
                        )
                # somehow this doesn't get updated
                chunks_cp[n].submobjects[1].text = new_text
            self.play(*anims)
        ...
        for g in range(4):
            chunks_cp = all_chunks_cp[g]
            out_texts = []
            new_text = str(int(chunks_cp[0].submobjects[1].text) + int(chunks[g][g].submobjects[1].text))
            out_texts.append(Text(new_text).scale(0.35).move_to(output[g][g]))
            anims.append(
                    AnimationGroup(
                        Transform(chunks_cp[0].submobjects[1].copy(),
                                  out_texts[-1]
                                  ),
                        Transform(chunks[g][g].submobjects[1].copy(),
                                  out_texts[-1].copy()
                                  ),
                        Transform(chunks_cp[0],
                                  Group(output[g][g].copy()),
                                  remover=True
                                  ),
                        )
                    )
            output[g][g].add(out_texts[-1])
        self.play(*anims)
        ...
        for g in range(4):
            ...
            for i in range(3):
                ...
                c = output[g][g].copy()
                chunks_cp.append(c)
                anims.append(c.animate.move_to(paths[g][n].get_start()))
            all_chunks_cp.append(chunks_cp)
        self.play(*anims)
        ...
        for g in range(4):
            ...
            for i in range(3):
                n = i+int(i >= g)
                c = chunks_cp[i]
                anims.append(MoveAlongPath(c, paths[g][n]))
        self.play(*anims)
        ...
        for g in range(4):
            for i in range(3):
                ...
                j = g-int(i < g)
                c = all_chunks_cp[n][j]
                anims.append(c.animate.move_to(symmem[g][n].get_center()))
        self.play(*anims)
        ...
        for g in range(4):
            for i in range(3):
                ...
                j = g-int(i < g)
                c = all_chunks_cp[n][j]
                anims.append(c.animate.set_color(BLUE, recurse=False).move_to(output[g][n]))
                        # AnimationGroup(
                        #     Transform(c.submobjects[0],
                        #               Text(new_text).scale(0.35).move_to(output[g][n])
                        #               ),
                        #     # Transform(c,
                        #     #           output[g][n].copy(),
                        #     #           remover=True
